...
void Explosion_init(Explosion* exp, float _x, float _y, float _r, ALLEGRO_COLOR _color){
    exp->x = _x;
    exp->y = _y;
    exp->radius = _r;
    exp->color = _color;
...
void Explosion_update(Explosion* exp){
    exp->radius += 0.5;
...
void Explosion_draw(Explosion* exp){
...
bool Explosion_remove(Explosion* exp){
    return exp->stopExp;
}
